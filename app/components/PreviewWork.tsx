...
// import odontologiaDigital from "@/public/videos/odontologiaDigital.mp4";
...
import { AlertDialog, AlertDialogTrigger, AlertDialogContent, AlertDialogHeader, AlertDialogTitle, AlertDialogDescription, AlertDialogFooter, AlertDialogCancel } from "@/app/components/ui/alert-dialog";
      ...
      return (
          <div className="w-[90%] md:w-[80%] mx-auto mt-10">
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10 md:mt-20">
                  {info.map((item, index) => (
                      <Card key={index} title={item.title} description={item.description} name={item.name} />
                  ))}
  ...
  const Card = ({ name, title, description }: { title: string; description: string; name: string }) => {
      return (
          <div className="bg-white flex flex-col items-center rounded-xl md:min-h-[20rem] min-h-[25rem] shadow-lg p-6 border border-zinc-200 transition-all duration-300 hover:shadow-2xl hover:scale-105">
              ...
              <h2 className={`text-center text-3xl md:text-4xl font-semibold text-[#00a2af] mx-5 md:mx-10 ${bebas.className}`}>
                  {title}
              </h2>
              <p className={`mt-5 text-center text-zinc-600 text-sm md:text-base flex-grow leading-relaxed ${montserrat.className}`}>
                  {description}
              </p>
